...
export default function EmployeesPage() {
  ...
  const loadDepartments = useCallback(async () => {
    ...
    try {
      ...
      const deptMap = deptList.reduce((acc: any, dept: any) => {
    ...
    try {
      ...
      if (filters.department_id && filters.department_id !== "all") {
        sortedData = sortedData.filter(
          (emp) => emp.department_id === filters.department_id
        );
      }
      if (filters.work_type && filters.work_type !== "all") {
        sortedData = sortedData.filter(
          (emp) => emp.work_type === filters.work_type
        );
      }
      if (filters.search) {
  ...
  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    router.push(
      `/dashboard/employees?page=${newPage}&sort=${sortField}&direction=${sortDirection}`
    );
  };
  ...
  const handleSort = (field: keyof Employee) => {
    ...
    router.push(
      `/dashboard/employees?page=${page}&sort=${field}&direction=${direction}`
    );
  };
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute(
      "download",
      `employees_export_${format(new Date(), "yyyy-MM-dd")}.csv`
    );
